...
async def cmd_start(message: types.Message):
    ...
    await message.answer(
        "Привет! 👋\n\n"
        "Я дам тебе набор материалов: мини-мануал, видео, чек-лист и таблицу KPI — по порядку.\n\n"
        "Начинаем с мини-мануала."
    )
...
async def cb_get_kpi(callback: CallbackQuery):
    ...
    if task is None or task.done():
        ...
        await callback.message.answer("Отлично — материалы выданы. Мы напомним тебе через 2 часа, чтобы предложить разбор кампании.")
    else:
...
async def schedule_delayed_message(user_id: int, delay_seconds: int = DELAY_SECONDS):
    ...
    try:
        await bot.send_message(user_id,
                               "⏰ Ты уже посмотрел видео? Хочешь разбор твоей рекламной кампании?",
...
async def skip_timer_or_handle_text(message: types.Message):
    ...
    if text == "жопа":
        ...
        try:
            await bot.send_message(user_id,
                                   "⏰ (Тест) Ты уже посмотрел видео? Хочешь разбор твоей рекламной кампании?",
                                   reply_markup=kb_start_quiz())
...
async def finalize_quiz(user_id: int):
    ...
    try:
        ...
        await bot.send_message(user_id,
                               ...
                               "Если хочешь, свяжись с нашим менеджером для разбора — он поможет подготовить план и тариф.",
